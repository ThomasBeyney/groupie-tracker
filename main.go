...
import ( // Importation des packages standard nécessaires
	"html/template" // Pour parser et exécuter les templates HTML
	"io"            // Pour copier les flux de données entre Reader et Writer
	"log"           // Pour le logging des messages et erreurs
	"net/http"      // Pour le serveur HTTP et les handlers
	"net/url"       // Pour encoder correctement les URLs
	"time"          // Pour gérer les délais et timeouts
)
...
func main() { // Point d’entrée de l’application
	...
	addr := ":8080"                                             // Adresse et port du serveur
	log.Printf("Starting server at http://localhost%s\n", addr) // Log de démarrage
	if err := http.ListenAndServe(addr, nil); err != nil {      // Lancement du serveur HTTP
		log.Fatalf("server error: %v", err) // Log fatal si erreur serveur
	}
}
